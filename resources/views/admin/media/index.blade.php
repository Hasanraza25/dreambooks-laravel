    ...
    <div class="box-body">
      ...
      <table class="table table-bordered">
        <form id="formView" class="formView">
          ...
          <tr>
            <td><input type="checkbox" name="checkAll[]" value="{{ $media->id }}" class="checkSingle"></td>
            <td>{{ $media->title }}</td>
            ...
            <td>
                ...
                <a href="{{ Route('media.destroy', $media->id) }}" class="btn btn-danger btn-flat btn-sm singleDelete"> <i class="fa fa-trash-o"></i></a>
            </td>
          </tr>
          @endforeach
        </form>
      ...
      <div class="row">
        <div class="col-sm-6">
            <span style="display:block;font-size:15px;line-height:34px;margin:20px 0;">
              <div>Showing {{($medias->currentpage()-1)*$medias->perpage()+1}} to {{$medias->currentpage()*$medias->perpage()}}
  ...
  $(document).ready(function() {
    ...
    $("#active_all_status").on('click', function(event) {
      ...
      if($('.checkSingle:checked').length > 0){
        ...
        $.get('{{ Route('media.active_all_status') }}', formSerials, function(data){
          if(data > 0){
    ...
    $("#deactive_all_status").on('click', function(event){
      ...
      if($(".checkSingle:checked").length > 0){
        ...
        $.get('{{ Route('media.deactive_all_status') }}', formSerials, function(data){
          if(data > 0){
            window.location.href = '/admin/media';
          }
        })
    ...
    $('#delete_all').on('click', function(e){
      ...
      if($('.checkSingle:checked').length > 0){
        if(confirm("Are you sure you want to delete this?")){
          ...
          $.get('{{ Route('media.delete_all') }}', formSerials, function(data){
            if(data > 0){
              window.location.href = '/admin/media';
            }
          })
